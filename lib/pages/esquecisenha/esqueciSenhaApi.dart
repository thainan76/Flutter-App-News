import 'dart:convert';
import 'package:app_spotway/pages/api_response.dart';
import 'package:app_spotway/pages/esquecisenha/esqueciSenha_model.dart';
import 'package:http/http.dart' as http;

class EsqueciSenhaApi {

  static Future<ApiResponse<EsqueciSenha>> esqueciSenha(String email) async {
    try {
      var url = 'https://api.spotway.com.br/esquecisenha';

      Map<String, String> headers = {"Content-type": "application/json"};

      Map params = {
        "headers" : {
            "usuario": "spotway",
            "token": "763a32e2-75a6-11ea-87d8-42010a9e0003"
        },
        "dados":{
          "email": email,
        }
      };

      String jsonLogin = json.encode(params);

      var response = await http.post(url, body: jsonLogin, headers: headers);

      print('Response status: ${response.statusCode}');
      print('Response body: ${response.body}');

      Map mapResponse = json.decode(response.body);
      //print(mapResponse);
      
      
      if (mapResponse['sucesso']) {
        final novaSenha = EsqueciSenha.fromJson(mapResponse);

        novaSenha.save();

        return ApiResponse.ok(novaSenha, mapResponse['mensagem']);
        //print('SALVOU!!');
      }

      return ApiResponse.error(mapResponse["mensagem"]);
    } catch (error, exception) {
      print("Erro no esqueci minha senha $error > $exception");

      return ApiResponse.error("Não foi possivel acessar o esqueci minha senha");
    }
  }


}